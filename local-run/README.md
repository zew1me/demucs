# Local uv Demucs runner

This directory isolates the developer-focused workflow and stores generated stems, but all Python dependencies are now defined in the repository-level `pyproject.toml`. Using [uv](https://github.com/astral-sh/uv) you can run Demucs locally without creating a traditional virtual environment.
...
```bash
uv sync          # from repo root or this folder; installs Demucs into .uv/
...
- This folder intentionally contains only documentation and generated assets; dependency tracking happens via the root `pyproject.toml`.
...
- Feel free to edit the root `pyproject.toml` to pin a specific Demucs version or add additional CLI tools you rely on.
